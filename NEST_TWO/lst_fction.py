...
key_path = "bucket_key.json"

credentials = service_account.Credentials.from_service_account_file(key_path)
client = storage.Client(credentials=credentials)

bucket_name = "nest-app"
bucket = client.bucket(bucket_name)


def extract_images_from_pdf(pdf_path, file_name: str):
    # Spécifier le chemin complet du dossier de sortie

    # Ouvrir le fichier PDF
    if isinstance(pdf_path, str):
        pdf_document = fitz.open(pdf_path)
    else:
        pdf_file = BytesIO(pdf_path.getvalue())
        pdf_document = fitz.open(stream=pdf_file)

    # Créer une liste pour stocker les noms des fichiers images
    list_image_bytes = []
    ...
    for page_number in range(pdf_document.page_count):
        ...
        for img_index, img_info in enumerate(images):
            ...
            # Obtenir l'image en utilisant l'index
            base_image = pdf_document.extract_image(image_index)
            image_bytes = base_image["image"]

            # Ajouter le nom du fichier à la liste
            list_image_bytes.append(image_bytes)

        largest_file_bytes = max(list_image_bytes)
        if list_image_bytes:
            blob = bucket.blob(f"parseur_ultime_aravinth/{file_name}.png")
            blob.upload_from_file(BytesIO(largest_file_bytes), content_type="image/png")

    # Fermer le document PDF
    pdf_document.close()


def get_urls_from_list_id_pdf_hashed(
    list_id_pdf_hashed: list[str], template_url_on_empty_picture: str
) -> list[str]:
    key_path = "bucket_key.json"
    folder = "parseur_ultime_aravinth"
    credentials = service_account.Credentials.from_service_account_file(key_path)
    client = storage.Client(credentials=credentials)
    ...
    list_blobs = [blob.name for blob in bucket.list_blobs(prefix=folder)]

    list_file_name = list(set(list_id_pdf_hashed).intersection(set(list_blobs)))

    list_signed_url = []

    for file_name in list_id_pdf_hashed:
        if file_name in list_file_name:
            blob = bucket.blob(file_name)
            signed_url = blob.generate_signed_url(
                expiration=datetime.timedelta(hours=1), method="GET"
            )
            list_signed_url.append(signed_url)
        else:
            list_signed_url.append(template_url_on_empty_picture)

    return list_signed_url
